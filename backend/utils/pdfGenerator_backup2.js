 ...
 * @param {String} template - The template to use (classic, executive, minimalist, creative, professional, academic, elegant)
...
function generateResumePDF(resumeData, studentInfo, stream, template = 'classic', layout = 'single-column') {
  ...
  // MODERN LAYOUT - Explicit positioning without margins
  const pageWidth = doc.page.width; // ~595 points
  const pageHeight = doc.page.height; // ~842 points
  
  // Two-column layout configuration
  const sidebarWidth = 180;
  const sidebarX = 0;
  const mainContentX = 200; // Clear separation from sidebar
  const mainContentWidth = pageWidth - mainContentX - 20;
  const contentStartY = 120;
  
  // Clean left sidebar with subtle gradient effect
  doc.rect(sidebarX, 100, sidebarWidth, pageHeight - 100)
     .fill(cardBg);

  // Add subtle inner shadow effect
  doc.rect(sidebarWidth - 2, 100, 2, pageHeight - 100)
     .fill('#f1f5f9');

  // Sidebar border
  doc.rect(sidebarWidth - 1, 100, 1, pageHeight - 100)
     .fill(borderColor);

  // DEBUG: Visual separator to clearly show layout divisions
  doc.rect(mainContentX - 10, 100, 2, pageHeight - 100)
     .fill('#ff0000'); // Red line to show main content boundary
  ...
  // Professional Summary - RIGHT SIDE POSITIONING
  if (resumeData.objective) {
    // Position at mainContentX (200)
    ...
    // Content positioned at mainContentX (200)
    doc.fontSize(11)
       .font('Helvetica')
       .fillColor(textMedium)
  ...
  // Education - RIGHT SIDE POSITIONING
