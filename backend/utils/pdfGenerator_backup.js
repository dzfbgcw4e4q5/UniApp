  ...
  try {
    switch (normalizedTemplate) {
      ...
      case 'newTemplate2Name':
        ...
        break;
      default:
        console.log(`Unknown template "${normalizedTemplate}", falling back to Modern template`);
        generateModernTemplate(safeResumeData, safeStudentInfo, stream, normalizedLayout);
    }
...
function generateModernTemplate(resumeData, studentInfo, stream, layout = 'two-column') {
  ...
  // Modern color palette - High contrast for better readability
  const primaryColor = '#2563eb';  // Strong blue
  const accentColor = '#06b6d4';   // Cyan accent
  const darkColor = '#1e3a8a';     // Dark blue
  const backgroundColor = '#ffffff'; // White background
  const textDark = '#111827';       // Very dark text
  const textMedium = '#374151';     // Medium gray text
  const textLight = '#6b7280';      // Light gray text
  const cardBg = '#f8fafc';         // Light card background
  ...
  // Modern header with strong contrast and better proportions
  doc.rect(0, 0, doc.page.width, 100)
     .fill(primaryColor);

  // Header accent line
  doc.rect(0, 95, doc.page.width, 5)
     .fill(accentColor);

  // Name with excellent contrast and proper sizing
  doc.fontSize(28)
     .font('Helvetica-Bold')
     .fillColor('#ffffff')
  ...
  // Contact info with high contrast and better spacing
  let contactY = 60;
  let contactInfo = [];
  ...
  if (studentInfo.branch) {
    contactInfo.push('🎓 ' + studentInfo.branch);
  }
  ...
  doc.fontSize(11)
     .font('Helvetica')
     .fillColor('#ffffff')
     .text(contactText, 40, contactY, { 
       width: doc.page.width - 80,
       align: 'center'
     });

  // OPTIMIZED MODERN LAYOUT - Properly balanced content distribution
  const sidebarWidth = 170;
  const mainContentX = sidebarWidth + 25;
  const mainContentWidth = doc.page.width - mainContentX - 30;
  const contentStartY = 120;
  
  // Clean left sidebar with subtle gradient effect
  doc.rect(0, 100, sidebarWidth, doc.page.height - 100)
     .fill(cardBg);

  // Add subtle inner shadow effect
  doc.rect(sidebarWidth - 2, 100, 2, doc.page.height - 100)
     .fill('#f1f5f9');

  // Sidebar border
  doc.rect(sidebarWidth - 1, 100, 1, doc.page.height - 100)
     .fill(borderColor);

  let sidebarY = contentStartY + 20;
  
  // CLEAN SIDEBAR CONTENT - Properly spaced sections
  if (resumeData.skills) {
    // Skills section header with accent line
    doc.rect(15, sidebarY, 3, 18)
       .fill(primaryColor);
    
    doc.fontSize(12)
       .font('Helvetica-Bold')
       .fillColor(textDark)
       .text('SKILLS', 25, sidebarY + 2);
    ...
    skillsLines.forEach(line => {
      ...
      // Clean skill item with proper spacing
      doc.fontSize(10)
         .font('Helvetica')
         .fillColor(textDark)
         .text('▪ ' + skill, 20, sidebarY, { 
  ...
  // Languages in sidebar with clean formatting
  if (resumeData.languages) {
    // Languages section header with accent line
    doc.rect(15, sidebarY, 3, 18)
       .fill(accentColor);
    ...
    languageLines.forEach(line => {
      ...
      // Clean language item with proper spacing
      doc.fontSize(10)
         .font('Helvetica')
         .fillColor(textMedium)
         .text('▪ ' + language, 20, sidebarY, { 
           width: sidebarWidth - 30,
           lineGap: 3
         });
  ...
  if (resumeData.certifications) {
    ...
    doc.fontSize(12)
       .font('Helvetica-Bold')
       .fillColor(textDark)
       .text('CERTIFICATIONS', 25, sidebarY + 2);
    ...
    certLines.forEach(line => {
      ...
      let cert = trimmedLine;
      if (cert.startsWith('•') || cert.startsWith('-')) {
        cert = cert.substring(1).trim();
      }
      
      doc.fontSize(9)
         .font('Helvetica')
         .fillColor(textMedium)
         .text('▪ ' + cert, 20, sidebarY, { 
           width: sidebarWidth - 30,
           lineGap: 2
         });
  ...
  let mainY = contentStartY + 20;

  // Professional Summary with high contrast and proper spacing
  if (resumeData.objective) {
    // Modern section header with strong accent
    doc.rect(mainContentX, mainY, 4, 22)
       .fill(primaryColor);
    
    doc.fontSize(14)
       .font('Helvetica-Bold')
       .fillColor(textDark)
       .text('PROFESSIONAL SUMMARY', mainContentX + 15, mainY + 3);
  ...
  // Education with clean design and proper spacing
  if (resumeData.education) {
    ...
    doc.fontSize(14)
       .font('Helvetica-Bold')
       .fillColor(textDark)
       .text('EDUCATION', mainContentX + 15, mainY + 3);
    
    mainY += 35;
    
    // Education content with proper formatting
    ...
    educationLines.forEach(line => {
      ...
      if (trimmedLine.includes('University') || trimmedLine.includes('College') || trimmedLine.includes('Institute') || trimmedLine.includes('School')) {
  ...
  // Experience with clear design and proper spacing
  if (resumeData.experience) {
    ...
    doc.fontSize(14)
       .font('Helvetica-Bold')
       .fillColor(textDark)
       .text('EXPERIENCE', mainContentX + 15, mainY + 3);
    
    mainY += 35;
    
    // Experience content with proper formatting
    ...
    experienceLines.forEach(line => {
      ...
      if (trimmedLine.includes('Company') || trimmedLine.includes('Organization') || trimmedLine.includes('Internship')) {
  ...
  // Projects with clear design
  if (resumeData.projects) {
    // Clear section header
    doc.rect(mainContentX, mainY, 4, 25)
       .fill(primaryColor);
    
    doc.fontSize(16)
       .font('Helvetica-Bold')
       .fillColor(textDark)
       .text('PROJECTS', mainContentX + 15, mainY + 4);
    ...
    projectLines.forEach(line => {
      ...
      if (isProjectTitle && !trimmedLine.startsWith('•') && !trimmedLine.startsWith('-')) {
        // Project title with clear accent
        ...
        doc.fontSize(12)
           .font('Helvetica-Bold')
           .fillColor(textDark)
           .text(trimmedLine, mainContentX + 15, mainY + 4, { width: mainContentWidth - 25 });
        mainY += 25;
        isProjectTitle = false;
      } else {
        // Project detail with clear text
        ...
        doc.fontSize(10)
           .font('Helvetica')
           .fillColor(textMedium)
           .text('• ' + detail, mainContentX + 15, mainY, { width: mainContentWidth - 25 });
        mainY = doc.y + 6;
      }
    });
  ...
  // Additional sections with clear styling
  ...
  additionalSections.forEach(section => {
    ...
    doc.fontSize(16)
       .font('Helvetica-Bold')
       .fillColor(textDark)
       .text(section.title, mainContentX + 15, mainY + 4);
    
    mainY += 40;
    
    // Content with clear styling
    doc.fontSize(11)
       .font('Helvetica')
       .fillColor(textMedium)
  ...
  // Clean footer with good contrast
  doc.rect(0, doc.page.height - 50, doc.page.width, 50)
     .fill(cardBg);

  doc.rect(0, doc.page.height - 50, doc.page.width, 2)
     .fill(borderColor);
...
function generateClassicTemplate(resumeData, studentInfo, stream, layout = 'single-column') {
  ...
  doc.fontSize(11)
     .font('Times-Roman')
     .fillColor(dark)
     .text(contactInfo.join(' • '), 0, 58, { align: 'center' });
  ...
  doc.fontSize(8).font('Times-Roman').fillColor(gold)
     .text(`${studentInfo.name} • Classic Resume • ${new Date().toLocaleDateString()}`, 0, pageHeight - 25, { align: 'center' });
  doc.end();
}
...
function generateMinimalistTemplate(resumeData, studentInfo, stream, layout = 'single-column') {
  ...
  doc.fontSize(11)
     .font('Helvetica')
     .fillColor(accentColor)
     .text(contactInfo.join(' • '), 0, currentY, { 
       width: doc.page.width,
       align: 'center'
     });
  currentY = doc.y + 40;
...
function generateCreativeTemplate(resumeData, studentInfo, stream, layout = 'single-column') {
  ...
  if (studentInfo.email) {
    doc.fontSize(12)
       .font('Helvetica')
       .fillColor('#ffffff')
       .text('📧 ' + studentInfo.email, 40, contactY);
    contactY += 20;
  }
  if (studentInfo.branch) {
    doc.fontSize(12)
       .font('Helvetica')
       .fillColor('#ffffff')
       .text('🎓 ' + studentInfo.branch, 40, contactY);
  }
...
function generateAcademicTemplate(resumeData, studentInfo, stream, layout = 'single-column') {
  ...
  doc.fontSize(12)
     .font('Times-Roman')
     .fillColor('#ffffff')
     .text(contactInfo.join(' • '), 0, contactY, { 
       width: doc.page.width,
       align: 'center'
     });
